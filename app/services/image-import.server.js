...
async function prepareMediaForItem(
  ...
  // Prefer the image that is already attached to this variant. Its alt text
  // may be empty (or may pre-date this importer), but it is still the image
  // the merchant expects us to replace for this SKU.
  const attachedImage =
    (variant.media || []).find(
      (media) =>
  ...
  const createdMedia =
    await createProductMedia(
      admin,
      {
        ...
        // An older product image can already have this SKU as its alt text.
        // Excluding the current IDs prevents the creation poll from returning
        // that old image and assigning it to the variant by mistake.
        existingMediaIds:
          productMedia.map(
            (media) => media.id
          ),
